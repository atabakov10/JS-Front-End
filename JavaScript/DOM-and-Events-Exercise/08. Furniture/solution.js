function solve() {
  ...
  function buyHandler() {
    ...
    for (const input of allCheckedInputs) {
      const tRow = input.parentElement.parentElement;
      const [fistColumn, secondColumn, thirdColumn, fourthColumn] = Array.from(tRow.children);
      let item = secondColumn.firstChild.textContent;
      boughtItems.push(item);
      let currentPrice = Number(thirdColumn.firstChild.textContent);
      totalPrice += currentPrice;
      let currentDecFactor = Number(fourthColumn.firstChild.textContent);
